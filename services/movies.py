...
async def retrieve_movies_from_omdb() -> list[MovieDB]:
    """
...
async def populate_database() -> list:
...
async def get_movies_from_firestore(limit: int = 10, page: int = 1):
...
async def get_movie_by_id(movie_id: str) -> MovieAPI:
...
async def get_movie_by_name(name: str) -> MovieAPI:
    movies_ref = db.collection(MOVIES_COLLECTION).where("title", "==", name)
